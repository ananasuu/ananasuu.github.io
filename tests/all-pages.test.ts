...
for (const page of pages) {
  ...
  test.describe(`Tests für ${page.title} (${page.url})`, () => {
    ...
    test.describe("a11y", { tag: [...tags, "@a11y"] }, () => {
      ...
      testA11y(
        `a11y test for ${page.url}`,
        async ({ page: playwright, makeAxeBuilder }) => {
    ...
    test.describe("performance", { tag: [...tags, "@performance"] }, () => {
      ...
      lighthouseTest(
        `lighthouse tests of ${page.url}`,
        async ({ page: playwright, port }) => {
          await playwright.goto(page.url);
    ...
    test.describe("functional", { tag: [...tags, "@functional"] }, () => {
      test.skip(
        ({ browserName }) => browserName.includes("dark"),
        "No need to run functional tests in dark mode",
      );

      test(`Seite lädt ohne Fehler: ${page.url}`, async ({
        page: playwright,
      }) => {
      ...
      test(`Navigation ist vorhanden: ${page.url}`, async ({
        page: playwright,
      }) => {
        await playwright.goto(page.url);
      ...
      test(`Footer ist vorhanden: ${page.url}`, async ({
        page: playwright,
      }) => {
        await playwright.goto(page.url);
